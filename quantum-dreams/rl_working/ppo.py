...
# 
            ...
            if config.get("LOGGING"):
                ...
                def callback(infos):
                    ...
                    # === 简单保存：三列（round, reward, fid）到 txt ===
                    try:
                        os.makedirs("per_action_logs", exist_ok=True)
                        txt_path = os.path.join("per_action_logs", "reward_fid_per_update.txt")
    
                        if not os.path.exists(txt_path):
                            with open(txt_path, "w") as f:
                                f.write("round\treward\tfid\n")
    
                        round_idx   = int(step) + 1
                        mean_reward = float(jnp.mean(info["reward"]))
                        mean_fid    = float(jnp.mean(info["fid"]))
                        with open(txt_path, "a") as f:
                            f.write(f"{round_idx}\t{mean_reward}\t{mean_fid}\n")
                    except Exception as e:
                        print(f"[warn] failed to write reward/fid txt: {e}")
